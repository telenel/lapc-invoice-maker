...
describe("checkAndSendReminders", () => {
  ...
  it("normalizes legacy one-digit reminder hours instead of aborting the reminder run", async () => {
    const tx = {
      $queryRaw: vi.fn().mockResolvedValue([{ acquired: true }]),
      event: {
        findMany: vi.fn().mockResolvedValue([
      ...
      notification: {
        create: vi.fn().mockResolvedValue({
          id: "notif-1",
          type: "EVENT_REMINDER",
          title: "Reminder: Legacy Time Event",
          message: "Starting at 9:00 \u2014 Room 101",
          quoteId: null,
          read: false,
          createdAt: new Date("2020-03-31T08:45:00.000Z"),
        }),
      },
      user: {
        findMany: vi.fn().mockResolvedValue([{ id: "user-1" }]),
