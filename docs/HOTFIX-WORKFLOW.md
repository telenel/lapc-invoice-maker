...
- The regular `main` deploy path no longer builds on the VPS. GitHub now publishes an exact-SHA image first and the VPS pulls that image. This hotfix lane intentionally keeps the direct build fallback for urgent low-risk fixes.
