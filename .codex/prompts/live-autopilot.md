...
- Poll the live snapshot with `npm run laportal:review:live:triage -- --json`.
...
- Use `npm run laportal:review:prompt -- --artifact .git/laportal/codex-review.live.json --batch <BATCH_ID>` to generate the exact worker prompt text.
...
- After completion, run `npm run laportal:review:triage` against the final artifact and reconcile any changes between the live snapshot and final report.
