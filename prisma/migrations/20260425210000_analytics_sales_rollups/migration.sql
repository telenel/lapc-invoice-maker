...
DROP MATERIALIZED VIEW IF EXISTS "analytics_sales_receipts_daily";
...
CREATE MATERIALIZED VIEW "analytics_sales_receipts_daily" AS
SELECT
  (st.process_date AT TIME ZONE 'America/Los_Angeles')::date AS sale_date,
  st.location_id,
...
CREATE UNIQUE INDEX analytics_sales_receipts_daily_unique_idx
  ON "analytics_sales_receipts_daily" ("sale_date", "location_id");
CREATE INDEX analytics_sales_receipts_daily_location_date_idx
  ON "analytics_sales_receipts_daily" ("location_id", "sale_date");
...
REVOKE ALL ON "analytics_sales_receipts_daily" FROM anon, authenticated;
