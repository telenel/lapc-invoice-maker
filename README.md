...
Docker Compose behind Traefik on [montalvo.io](https://montalvo.io). GitHub Actions runs formal `ship-check` and `actionlint` checks on PRs and pushes, and enables native GitHub auto-merge on PRs when `AUTOMERGE_PAT` is configured. Production deploys are intentionally manual so multiple short-lived PRs can merge into `main` before the VPS rebuilds once for the chosen release SHA. Trigger the `Deploy to VPS` workflow from GitHub Actions or run `npm run deploy:vps` to deploy latest `main`; set `DEPLOY_REF=<branch-or-sha>` to ship another pushed ref through the same exact-image path. The deploy workflow builds a single exact-SHA image in GitHub, publishes it to GHCR, then SSHes to the VPS so the server pulls that exact image and verifies it live. The workflow forwards its short-lived GHCR credentials over SSH, so regular GitHub-driven deploys do not need a long-lived registry token stored on the VPS.
