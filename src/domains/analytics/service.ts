...
export const analyticsService = {
  async getFinanceAnalytics(filters: AnalyticsFilters): Promise<FinanceAnalytics> {
    const range = resolveDateRange(filters);
    return analyticsRepository.findFinanceAnalytics(range);
  },

  async getOperationsAnalytics(filters: AnalyticsFilters): Promise<OperationsAnalytics> {
    const range = resolveDateRange(filters);
    const operationsSnapshot = await analyticsRepository.findOperationsSnapshot(range);
    return buildOperationsAnalytics(range, operationsSnapshot ?? EMPTY_OPERATIONS_SNAPSHOT);
  },

  async getAnalytics(filters: AnalyticsFilters): Promise<AnalyticsResponse> {
    const range = resolveDateRange(filters);
    const [finance, operationsSnapshot] = await Promise.all([
      analyticsRepository.findFinanceAnalytics(range),
