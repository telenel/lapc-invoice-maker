...
import { accumulateProductSummary } from "@/domains/product/summary";
import type {
...
const BASE_SUMMARY_SELECT = [
  ...
  "txns_1y",
  "last_sale_date",
  "sales_aggregates_computed_at",
].join(",");

const DERIVED_SUMMARY_SELECT = [
  BASE_SUMMARY_SELECT,
...
interface SummaryChunkQuery extends SummaryQueryChain {
  order(
    column: string,
...
function parseAnalysisWindow(searchParams: URLSearchParams): ProductAnalysisWindow {
  const raw = searchParams.get("analysisWindow");
  if (raw === "30d" || raw === "90d" || raw === "1y") return raw;
  return "1y";
}

function applySummaryFilters<T extends SummaryQueryChain>(
  query: T,
...
function mapLatestSyncStatus(status: string | null | undefined): ProductSummaryResponse["freshness"]["latestSyncStatus"] {
  if (status === "ok" || status === "partial" || status === "failed") return status;
  return "unknown";
}

async function fetchAllSummaryRows(filters: ProductFilters): Promise<ProductSummaryRow[]> {
  const supabase = getSupabaseAdminClient();
  const plan = buildProductQueryPlan(filters);
  const select = plan.source === "products_with_derived" ? DERIVED_SUMMARY_SELECT : BASE_SUMMARY_SELECT;
  const rows: ProductSummaryRow[] = [];

  for (let offset = 0; ; offset += CHUNK_SIZE) {
    const query = applySummaryFilters(
      supabase
      .from(plan.source)
      .select(select)
      .in("item_type", TAB_ITEM_TYPES[filters.tab]) as unknown as SummaryChunkQuery,
      filters,
      plan,
    );
    ...
    const chunk = (data ?? []) as ProductSummaryRow[];
    rows.push(...chunk);
...
export const GET = withAuth(async (request: NextRequest) => {
  ...
  const analysisWindow = parseAnalysisWindow(searchParams);
  const [rows, latestRun] = await Promise.all([
    fetchAllSummaryRows(filters),
  ...
  const summary = accumulateProductSummary(rows, analysisWindow);

  return NextResponse.json({
    ...summary,
    freshness: {
      ...summary.freshness,
      latestSyncStatus: mapLatestSyncStatus(latestRun?.status),
      latestSyncCompletedAt: latestRun?.completedAt?.toISOString() ?? null,
    },
  });
});
